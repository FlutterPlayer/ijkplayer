...
static int packet_queue_get_or_buffering(FFPlayer *ffp, PacketQueue *q,
...
static int convert_image(FFPlayer *ffp, AVFrame *src_frame,
                         int64_t src_frame_pts, int width, int height) {
  ...
  if (is->viddec.avctx) {
    av_reduce(&display_aspect_ratio.num, &display_aspect_ratio.den,
              is->viddec.avctx->width *
                  (int64_t)is->viddec.avctx->sample_aspect_ratio.num,
              is->viddec.avctx->height *
                  (int64_t)is->viddec.avctx->sample_aspect_ratio.den,
              1024 * 1024);
  ...
  if ((int)(origin_dar * 100) != (int)(dar * 100)) {
    ...
    av_log(NULL, AV_LOG_INFO,
           "%s img_info->width = %d, img_info->height = %d\n", __func__,
  ...
  if (!img_info->frame_img_convert_ctx) {
    img_info->frame_img_convert_ctx =
        sws_getContext(width, height, src_frame->format, dst_width, dst_height,
  ...
  if (!img_info->frame_img_codec_ctx) {
    ...
    img_info->frame_img_codec_ctx->time_base.num =
        ffp->is->video_st->time_base.num;
    img_info->frame_img_codec_ctx->time_base.den =
        ffp->is->video_st->time_base.den;
  ...
  ret = av_image_fill_arrays(dst_frame->data, dst_frame->linesize, buffer,
  ...
  ret = avcodec_encode_video2(img_info->frame_img_codec_ctx, &avpkt, dst_frame,
                              &got_packet);
  ...
  if (ret >= 0 && got_packet > 0) {
    ...
    if (img_info->count <= 0)
      ffp_notify_msg4(ffp, FFP_MSG_GET_IMG_STATE, (int)src_frame_pts, 1,
                      file_name, file_name_length);
    else
      ffp_notify_msg4(ffp, FFP_MSG_GET_IMG_STATE, (int)src_frame_pts, 0,
                      file_name, file_name_length);
    ...
    if (d->queue->serial == d->pkt_serial) {
      do {
        ...
        case AVMEDIA_TYPE_VIDEO:
          ...
          if (ret >= 0) {
            ffp->stat.vdps = SDL_SpeedSamplerAdd(
                &ffp->vdps_sampler, FFP_SHOW_VDPS_AVCODEC, "vdps[avcodec]");
          ...
          if (ret >= 0) {
            ...
            if (frame->pts != AV_NOPTS_VALUE)
              frame->pts = av_rescale_q(
                  frame->pts, av_codec_get_pkt_timebase(d->avctx), tb);
...
static void ffp_clock_msg_notify_cycle(FFPlayer *ffp, int64_t time_ms) {
  if (ffp->enable_position_notify &&
      (time_ms < 0 ||
       time_ms - ffp->clock_notify_time > ffp->pos_update_interval)) {
...
static void video_image_display2(FFPlayer *ffp) {
  ...
  if (vp->bmp) {
        ...
        if (vp->pts >= sp->pts + ((float)sp->sub.start_display_time / 1000)) {
          if (!sp->uploaded) {
            if (sp->sub.num_rects > 0) {
              ...
              ffp_notify_msg4(ffp, FFP_MSG_TIMED_TEXT, 0, 0, buffered_text,
                              sizeof(buffered_text));
            }
            sp->uploaded = 1;
          }
        }
    ...
    ffp->stat.vfps = SDL_SpeedSamplerAdd(&ffp->vfps_sampler,
                                         FFP_SHOW_VFPS_FFPLAY, "vfps[ffplay]");
    ...
    if (is->latest_video_seek_load_serial == vp->serial) {
      ...
      int latest_video_seek_load_serial =
          InterlockedExchangeAcquire(&(is->latest_video_seek_load_serial), -1);
#else
      int latest_video_seek_load_serial = __atomic_exchange_n(
          &(is->latest_video_seek_load_serial), -1, memory_order_seq_cst);
#endif
      if (latest_video_seek_load_serial == vp->serial) {
        ffp->stat.latest_seek_load_duration =
            (av_gettime() - is->latest_seek_load_start_at) / 1000;
...
static double compute_target_delay(FFPlayer *ffp, double delay,
                                   VideoState *is) {
...
static void video_refresh(FFPlayer *opaque, double *remaining_time) {
  ...
  retry:
    if (frame_queue_nb_remaining(&is->pictq) == 0) {
      ...
      if (is->paused &&
          (ffp->first_video_frame_rendered || !ffp->cover_after_prepared))
        goto display;
...
static int queue_picture(FFPlayer *ffp, AVFrame *src_frame, double pts,
    ...
    if (!isnan(pts)) {
      ...
      if ((pts * 1000 * 1000 < is->seek_pos) || deviation > MAX_DEVIATION) {
        ...
        if (is->drop_vframe_count == 0) {
          ...
          av_log(NULL, AV_LOG_INFO,
                 "video accurate_seek start, is->seek_pos=%" PRId64
                 ", pts=%lf, is->accurate_seek_time = %" PRId64 "\n",
        ...
        if ((now - is->accurate_seek_start_time) <=
          ...
          av_log(NULL, AV_LOG_WARNING,
                 "video accurate_seek is error, is->drop_vframe_count=%d, now "
                 "= %" PRId64 ", pts = %lf\n",
                 is->drop_vframe_count, now, pts);
          video_accurate_seek_fail =
              1; // if KEY_FRAME interval too big, disable accurate seek
        }
      } else {
        av_log(NULL, AV_LOG_INFO,
               "video accurate_seek is ok, is->drop_vframe_count =%d, "
               "is->seek_pos=%" PRId64 ", pts=%lf\n",
  ...
  if (!vp->bmp || !vp->allocated || vp->width != src_frame->width ||
    ...
    if (vp->width != src_frame->width || vp->height != src_frame->height)
      ffp_notify_msg3(ffp, FFP_MSG_VIDEO_SIZE_CHANGED, src_frame->width,
                      src_frame->height);
  ...
  if (vp->bmp) {
    ...
    av_image_copy(data, linesize, (const uint8_t **)src_frame->data,
                  src_frame->linesize, src_frame->format, vp->width,
                  vp->height);
#else
...
static int configure_video_filters(FFPlayer *ffp, AVFilterGraph *graph,
                                   VideoState *is, const char *vfilters,
                                   AVFrame *frame) {
...
static int configure_audio_filters(FFPlayer *ffp, const char *afilters,
  ...
  if (fabsf(ffp->pf_playback_rate) > 0.00001 &&
    ...
    av_strlcatf(afilters_args, sizeof(afilters_args), "atempo=%f",
                ffp->pf_playback_rate);
  }
#endif

  if ((ret = configure_filtergraph(is->agraph,
                                   afilters_args[0] ? afilters_args : NULL,
...
static int audio_thread(void *arg) {
  ...
  do {
    ...
    if (got_frame) {
      ...
      if (ffp->enable_accurate_seek && is->audio_accurate_seek_req &&
          !is->seek_req) {
        frame_pts =
        ...
        if (!isnan(frame_pts)) {
          ...
          deviation =
              llabs((int64_t)(audio_clock * 1000 * 1000) - is->seek_pos);
          if ((audio_clock * 1000 * 1000 < is->seek_pos) ||
            ...
            if (is->drop_aframe_count == 0) {
              ...
              av_log(
                  NULL, AV_LOG_INFO,
                  "audio accurate_seek start, is->seek_pos=%" PRId64
                  ", audio_clock=%lf, is->accurate_seek_start_time = %" PRId64
                  "\n",
            ...
            if (!is->video_accurate_seek_req && is->video_stream >= 0 &&
            ...
            if (audio_seek_pos == is->seek_pos) {
              ...
              if (audio_seek_pos == is->seek_pos &&
                  is->video_accurate_seek_req && !is->abort_request) {
        ...
        if (audio_accurate_seek_fail) {
          av_log(NULL, AV_LOG_INFO,
                 "audio accurate_seek is error, is->drop_aframe_count=%d, now "
                 "= %" PRId64 ", audio_clock = %lf\n",
      ...
      reconfigure = cmp_audio_fmts(is->audio_filter_src.fmt,
                    ...
                    is->auddec.pkt_serial != last_serial || ffp->af_changed;
...
#else
  ffp_notify_msg2(ffp, FFP_MSG_VIDEO_ROTATION_CHANGED,
                  ffp_get_video_rotate_degrees(ffp));
#endif
  ...
  for (;;) {
    ...
    if (ffp->get_frame_mode) {
      ...
      if (dst_pts < 0) {
        ...
        dst_pts +=
            (ffp->get_img_info->end_time - ffp->get_img_info->start_time) /
            (ffp->get_img_info->num - 1);
      }
      ...
      if (pts >= dst_pts) {
        while (retry_convert_image <= MAX_RETRY_CONVERT_IMAGE) {
          ret = convert_image(ffp, frame, (int64_t)pts, frame->width,
                              frame->height);
          if (!ret) {
            convert_frame_count++;
            break;
          ...
          av_log(NULL, AV_LOG_ERROR,
                 "convert image error retry_convert_image = %d\n",
                 retry_convert_image);
        }
        ...
        if (ret || ffp->get_img_info->count <= 0) {
          if (ret) {
            ...
            av_log(NULL, AV_LOG_INFO,
                   "convert image complete convert_frame_count = %d\n",
                   convert_frame_count);
          }
          goto the_end;
        }
    ...
    if (last_w != frame->width || last_h != frame->height ||
        ...
        ffp->vf_changed || last_vfilter_idx != is->vfilter_idx) {
      ...
      av_log(
          ...
          (const char *)av_x_if_null(av_get_pix_fmt_name(frame->format),
                                     "none"),
      ...
      if ((ret = configure_video_filters(
               ffp, graph, is,
               ffp->vfilters_list ? ffp->vfilters_list[is->vfilter_idx] : NULL,
               frame)) < 0) {
        // FIXME: post error
...
static int subtitle_thread(void *arg) {
  ...
  for (;;) {
    ...
    if ((got_subtitle =
             decoder_decode_frame(ffp, &is->subdec, NULL, &sp->sub)) < 0)
      break;
  ...
  if (af->frame->format != is->audio_src.fmt ||
    ...
    if (!is->swr_ctx) {
      av_log(NULL, AV_LOG_ERROR,
             ...
             af->frame->channels, is->audio_tgt.freq,
    ...
    if (af->frame->channel_layout == AV_CH_LAYOUT_5POINT1_BACK)
      av_opt_set_double(is->swr_ctx, "center_mix_level",
                        ffp->preset_5_1_center_mix_level, 0);
  ...
  if (is->swr_ctx) {
    ...
    if (ffp->soundtouch_enable && ffp->pf_playback_rate != 1.0f &&
      ...
      for (int i = 0; i < (resampled_data_size / 2); i++) {
        is->audio_new_buf[i] =
            (is->audio_buf1[i * 2] | (is->audio_buf1[i * 2 + 1] << 8));
      }

      int ret_len = ijk_soundtouch_translate(
          is->handle, is->audio_new_buf, (float)(ffp->pf_playback_rate),
          (float)(1.0f / ffp->pf_playback_rate), resampled_data_size / 2,
          bytes_per_sample, is->audio_tgt.channels, af->frame->sample_rate);
      if (ret_len > 0) {
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len) {
  ...
  if (!isnan(is->audio_clock)) {
    set_clock_at(&is->audclk,
                 is->audio_clock -
                     (double)(is->audio_write_buf_size) /
                         is->audio_tgt.bytes_per_sec -
                     SDL_AoutGetLatencySeconds(ffp->aout),
                 is->audio_clock_serial, ffp->audio_callback_time / 1000000.0);
  ...
  if (is->latest_audio_seek_load_serial == is->audio_clock_serial) {
    ...
    int latest_audio_seek_load_serial =
        InterlockedExchangeAcquire(&(is->latest_audio_seek_load_serial), -1);
#else
    int latest_audio_seek_load_serial = __atomic_exchange_n(
        &(is->latest_audio_seek_load_serial), -1, memory_order_seq_cst);
#endif
...
static int audio_open(FFPlayer *opaque, int64_t wanted_channel_layout,
  ...
  SDL_AoutSetDefaultLatencySeconds(
      ffp->aout, ((double)(2 * spec.size)) / audio_hw_params->bytes_per_sec);
  return spec.size;
}
...
static int stream_component_open(FFPlayer *ffp, int stream_index) {
  ...
  opts = filter_codec_opts(ffp->codec_opts, avctx->codec_id, ic,
...
#if CONFIG_AVFILTER
  {
    ...
    if ((ret = decoder_start(&is->auddec, audio_thread, ffp, "ff_audio_dec")) <
        0)
      goto out;
  ...
  case AVMEDIA_TYPE_VIDEO:
    ...
    if ((ret = decoder_start(&is->viddec, video_thread, ffp, "ff_video_dec")) <
        0)
      goto out;
    ...
    if (is->is_video_high_fps) {
      ...
      avctx->skip_loop_filter =
          FFMAX(avctx->skip_loop_filter, AVDISCARD_NONREF);
  ...
  case AVMEDIA_TYPE_SUBTITLE:
    ...
    if ((ret = decoder_start(&is->subdec, subtitle_thread, ffp,
                             "ff_subtitle_dec")) < 0)
      goto out;
    break;
  default:
...
static int stream_has_enough_packets(AVStream *st, int stream_id,
                                     PacketQueue *queue, int min_frames) {
...
static int read_thread(void *arg) {
  ...
  if (!av_dict_get(ffp->format_opts, "scan_all_pmts", NULL,
                   AV_DICT_MATCH_CASE)) {
    av_dict_set(&ffp->format_opts, "scan_all_pmts", "1",
  ...
  if (ffp->skip_calc_frame_rate) {
    av_dict_set_int(&ic->metadata, "skip-calc-frame-rate",
                    ffp->skip_calc_frame_rate, 0);
    av_dict_set_int(&ffp->format_opts, "skip-calc-frame-rate",
                    ffp->skip_calc_frame_rate, 0);
  }
  ...
  if (st_index[AVMEDIA_TYPE_VIDEO] >= 0)
    ijkmeta_set_int64_l(ffp->meta, IJKM_KEY_VIDEO_STREAM,
                        st_index[AVMEDIA_TYPE_VIDEO]);
  if (st_index[AVMEDIA_TYPE_AUDIO] >= 0)
    ijkmeta_set_int64_l(ffp->meta, IJKM_KEY_AUDIO_STREAM,
                        st_index[AVMEDIA_TYPE_AUDIO]);
  if (st_index[AVMEDIA_TYPE_SUBTITLE] >= 0)
    ijkmeta_set_int64_l(ffp->meta, IJKM_KEY_TIMEDTEXT_STREAM,
  ...
  if (is->video_st && is->video_st->codecpar) {
    AVCodecParameters *codecpar = is->video_st->codecpar;
    ffp_notify_msg3(ffp, FFP_MSG_VIDEO_SIZE_CHANGED, codecpar->width,
                    codecpar->height);
  ...
  for (;;) {
    ...
    if ((!is->paused || completed) &&
      ...
      if (ffp->loop != 1 && (!ffp->loop || --ffp->loop)) {
        stream_seek(is, ffp->start_time != AV_NOPTS_VALUE ? ffp->start_time : 0,
                    0, 0);
        ...
        if (completed) {
          ...
          if (ffp->error) {
            ...
            ffp_notify_msg4(ffp, FFP_MSG_ERROR, ffp->error, 0, error_msg,
                            AV_ERROR_MAX_STRING_SIZE);
          } else {
            if (ffp->enable_position_notify) {
              int64_t position = ffp_get_duration_l(ffp);
              ffp_notify_msg2(ffp, FFP_MSG_CURRENT_POSITION_UPDATE,
                              (int)position);
            }
    ...
    if (ffp->packet_buffering) {
      ...
      if ((!ffp->first_video_frame_rendered && is->video_st) ||
          (!ffp->first_audio_frame_rendered && is->audio_st)) {
        if (abs((int)(io_tick_counter - prev_io_tick_counter)) >
            FAST_BUFFERING_CHECK_PER_MILLISECONDS) {
...
fail:
  ...
  if (!ffp->prepared || !is->abort_request) {
    ...
    ffp_notify_msg4(ffp, FFP_MSG_ERROR, ffp->last_error, 0, error_msg,
                    AV_ERROR_MAX_STRING_SIZE);
  }
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename,
  ...
  ffp->startup_volume = av_clip(SDL_MIX_MAXVOLUME * ffp->startup_volume / 100,
                                0, SDL_MIX_MAXVOLUME);
  ...
  // is->video_refresh_tid = SDL_CreateThreadEx(&is->_video_refresh_tid,
  // video_refresh_thread, ffp, "ff_vout");
  is->video_refresh_tid =
      SDL_CreateThread(video_refresh_thread, "ff_vout", ffp);
  ...
  if (ffp->async_init_decoder && !ffp->video_disable && ffp->video_mime_type &&
      strlen(ffp->video_mime_type) > 0 && ffp->mediacodec_default_name &&
...
static void ffp_log_callback_brief(void *ptr, int level, const char *fmt,
                                   va_list vl) {
...
void ffp_io_stat_complete_register(
    void (*cb)(const char *url, int64_t read_bytes, int64_t total_size,
               int64_t elpased_time, int64_t total_duration)) {
  // avijk_io_stat_complete_register(cb);
}

static const char *ffp_context_to_name(void *ptr) { return "FFPlayer"; }

static void *ffp_context_child_next(void *obj, void *prev) { return NULL; }
...
static const char *ijk_version_info() { return IJKPLAYER_VERSION; }
...
static int app_func_event(AVApplicationContext *h, int message, void *data,
                          size_t size) {
  if (!h || !h->opaque || !data)
    return 0;
  ...
  } else if (message == AVAPP_EVENT_ASYNC_STATISTIC &&
             sizeof(AVAppAsyncStatistic) == size) {
...
static int ijkio_app_func_event(IjkIOApplicationContext *h, int message,
  ...
  if (message == IJKIOAPP_EVENT_CACHE_STATISTIC &&
      sizeof(IjkIOAppCacheStatistic) == size) {
    IjkIOAppCacheStatistic *statistic =
        (IjkIOAppCacheStatistic *)(intptr_t)data;
...
void ffp_set_frame_at_time(FFPlayer *ffp, const char *path, int64_t start_time,
                           int64_t end_time, int num, int definition) {
  if (!ffp->get_img_info) {
...
void *ffp_set_ijkio_inject_opaque(FFPlayer *ffp, void *opaque) {
  ...
  ffp_set_option_int(ffp, FFP_OPT_CATEGORY_FORMAT, "ijkiomanager",
                     (int64_t)(intptr_t)ffp->ijkio_manager_ctx);
...
void *ffp_set_inject_opaque(FFPlayer *ffp, void *opaque) {
  ...
  ffp_set_option_int(ffp, FFP_OPT_CATEGORY_FORMAT, "ijkapplication",
                     (int64_t)(intptr_t)ffp->app_ctx);

  ffp->app_ctx->func_on_app_event = app_func_event;
  return prev_weak_thiz;
}

void ffp_set_option(FFPlayer *ffp, int opt_category, const char *name,
                    const char *value) {
  if (!ffp)
    return;
...
void ffp_set_option_int(FFPlayer *ffp, int opt_category, const char *name,
                        int64_t value) {
  if (!ffp)
    return;
...
static void ffp_show_version_str(FFPlayer *ffp, const char *module,
                                 const char *version) {
  av_log(ffp, AV_LOG_INFO, "%-*s: %s\n", FFP_VERSION_MODULE_NAME_LENGTH, module,
         version);
}

static void ffp_show_version_int(FFPlayer *ffp, const char *module,
                                 unsigned version) {
  av_log(ffp, AV_LOG_INFO, "%-*s: %u.%u.%u\n", FFP_VERSION_MODULE_NAME_LENGTH,
         module, (unsigned int)IJKVERSION_GET_MAJOR(version),
 ...
 */
static void ffp_get_snap_shot(void *opaque, uint8_t *pixels, int width,
                              int height) {
  FFPlayer *ffp = opaque;
...
long ffp_get_current_position_l(FFPlayer *ffp) {
  ...
  // If using REAL time and not ajusted, then return the real pos as calculated
  // from the stream the use case for this is primarily when using a custom
  // non-seekable data source that starts with a buffer that is NOT the start of
  // the stream.  We want the get_current_position to return the time in the
  // stream, and not the player's internal clock.
...
int ffp_packet_queue_init(PacketQueue *q) { return packet_queue_init(q); }

void ffp_packet_queue_destroy(PacketQueue *q) { packet_queue_destroy(q); }

void ffp_packet_queue_abort(PacketQueue *q) { packet_queue_abort(q); }

void ffp_packet_queue_start(PacketQueue *q) { packet_queue_start(q); }

void ffp_packet_queue_flush(PacketQueue *q) { packet_queue_flush(q); }

int ffp_packet_queue_get(PacketQueue *q, AVPacket *pkt, int block,
                         int *serial) {
  return packet_queue_get(q, pkt, block, serial);
}

int ffp_packet_queue_get_or_buffering(FFPlayer *ffp, PacketQueue *q,
                                      AVPacket *pkt, int *serial,
                                      int *finished) {
...
void ffp_frame_queue_push(FrameQueue *f) { frame_queue_push(f); }
...
double ffp_get_master_clock(VideoState *is) { return get_master_clock(is); }
...
void ffp_track_statistic_l(FFPlayer *ffp, AVStream *st, PacketQueue *q,
                           FFTrackCacheStatistic *cache) {
  assert(cache);
...
void ffp_check_buffering_l(FFPlayer *ffp) {
  VideoState *is = ffp->is;
  int hwm_in_ms = ffp->dcc.current_high_water_mark_in_ms; // use fast water mark
                                                          // for first loading
  ...
  if (is->audio_st)
    audio_time_base_valid =
        is->audio_st->time_base.den > 0 && is->audio_st->time_base.num > 0;
  if (is->video_st)
    video_time_base_valid =
        is->video_st->time_base.den > 0 && is->video_st->time_base.num > 0;
  ...
  if (hwm_in_ms > 0) {
    ...
    if (is->audio_st && audio_time_base_valid) {
      ...
      int audio_cached_percent =
          (int)av_rescale(audio_cached_duration, 1005, hwm_in_ms * 10);
      av_log(ffp, AV_LOG_DEBUG,
             "audio cache=%%%d milli:(%d/%d) bytes:(%d/%d) packet:(%d/%d)\n",
             audio_cached_percent, (int)audio_cached_duration, hwm_in_ms,
             is->audioq.size, hwm_in_bytes, is->audioq.nb_packets, MIN_FRAMES);
#endif
    }
...
#ifdef FFP_SHOW_DEMUX_CACHE
      int video_cached_percent =
          (int)av_rescale(video_cached_duration, 1005, hwm_in_ms * 10);
      av_log(ffp, AV_LOG_DEBUG,
             "video cache=%%%d milli:(%d/%d) bytes:(%d/%d) packet:(%d/%d)\n",
             video_cached_percent, (int)video_cached_duration, hwm_in_ms,
             is->videoq.size, hwm_in_bytes, is->videoq.nb_packets, MIN_FRAMES);
#endif
    }

    if (video_cached_duration > 0 && audio_cached_duration > 0) {
      cached_duration_in_ms =
          (int)IJKMIN(video_cached_duration, audio_cached_duration);
    ...
    if (cached_duration_in_ms >= 0) {
      buf_time_position =
          ffp_get_current_position_l(ffp) + cached_duration_in_ms;
      ffp->playable_duration_ms = buf_time_position;

      buf_time_percent =
          (int)av_rescale(cached_duration_in_ms, 1005, hwm_in_ms * 10);
#ifdef FFP_SHOW_DEMUX_CACHE
      av_log(ffp, AV_LOG_DEBUG, "time cache=%%%d (%d/%d)\n", buf_time_percent,
             cached_duration_in_ms, hwm_in_ms);
#endif
#ifdef FFP_NOTIFY_BUF_TIME
      ffp_notify_msg3(ffp, FFP_MSG_BUFFERING_TIME_UPDATE, cached_duration_in_ms,
                      hwm_in_ms);
#endif
    }
  }
  ...
  if (hwm_in_bytes > 0) {
    ...
    av_log(ffp, AV_LOG_DEBUG, "size cache=%%%d (%d/%d)\n", buf_size_percent,
           cached_size, hwm_in_bytes);
#endif
#ifdef FFP_NOTIFY_BUF_BYTES
    ffp_notify_msg3(ffp, FFP_MSG_BUFFERING_BYTES_UPDATE, cached_size,
                    hwm_in_bytes);
#endif
  }
  ...
  if (buf_percent) {
    ...
    av_log(ffp, AV_LOG_DEBUG, "buf pos=%" PRId64 ", %%%d\n", buf_time_position,
           buf_percent);
#endif
    ffp_notify_msg3(ffp, FFP_MSG_BUFFERING_UPDATE, (int)buf_time_position,
                    buf_percent);
  }
  ...
  if (need_start_buffering) {
    ...
    if (is->buffer_indicator_queue &&
        is->buffer_indicator_queue->nb_packets > 0) {
      if ((is->audioq.nb_packets >= MIN_MIN_FRAMES || is->audio_stream < 0 ||
           is->audioq.abort_request) &&
          (is->videoq.nb_packets >= MIN_MIN_FRAMES || is->video_stream < 0 ||
...
void ffp_set_video_codec_info(FFPlayer *ffp, const char *module,
  ...
  ffp->video_codec_info =
      av_asprintf("%s, %s", module ? module : "", codec ? codec : "");
  av_log(ffp, AV_LOG_INFO, "VideoCodec: %s\n", ffp->video_codec_info);
}

void ffp_set_audio_codec_info(FFPlayer *ffp, const char *module,
                              const char *codec) {
  av_freep(&ffp->audio_codec_info);
...
void ffp_set_subtitle_codec_info(FFPlayer *ffp, const char *module,
                                 const char *codec) {
  av_freep(&ffp->subtitle_codec_info);
...
int64_t ffp_get_property_int64(FFPlayer *ffp, int id, int64_t default_value) {
  ...
  case FFP_PROP_INT64_TCP_SPEED:
    return ffp ? SDL_SpeedSampler2GetSpeed(&ffp->stat.tcp_read_sampler)
               : default_value;
...
int ffp_start_record(FFPlayer *ffp, const char *file_name) {
  ...
  if (!is || !is->ic || is->paused ||
      is->abort_request) { // 没有上下文，或者上下文已经停止
  ...
  for (int i = 0; i < is->ic->nb_streams; i++) {
    ...
    if (!out_stream->codecpar->block_align)
      out_stream->codecpar->block_align =
          out_stream->codecpar->channels *
              av_get_bits_per_sample(out_stream->codecpar->codec_id) >>
          3;
    ...
    if (ret < 0) {
      av_log(NULL, AV_LOG_ERROR,
             "Failed to copy context from input to output stream codec "
             "context, error:%d",
             ret);
      goto end;
    }
...
int ffp_record_file(FFPlayer *ffp, AVPacket *packet) {
  ...
  if (ffp->is_record) {
    ...
    AVPacket *pkt = (AVPacket *)av_malloc(
        sizeof(AVPacket)); // 与看直播的 AVPacket分开，不然卡屏
    av_new_packet(pkt, 0);
    //        av_packet_alloc();
